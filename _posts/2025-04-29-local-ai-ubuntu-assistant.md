---
layout: default # <--- 已修改为 'default'，与你的博客主页和列表页保持一致
title: "告别API与大模型负担：在Ubuntu上打造你的专属本地AI命令助手！"
date: 2025-04-29 10:00:00 +0800 # 使用了你文件名中的日期，可按需调整时间
categories: [技术分享天算AI (Natural Algorithm)** 出品的开源项目——**local-ai-ubuntu**。这是一个专为 Ubuntu 用户设计的本地 AI 命令助手，让你在享受自然语言操控电脑的便捷同时，彻底告别 API 成本和大型模型的负担！
...
*   **常用工具:** "ping百度",, Ubuntu, AI, 开源] # (可选) 博文分类
...
*   **AI 般的便捷:** 用自然语言下达指令，操作电脑从未如此丝滑。
*   **零 API 成本:** 完全    克隆下来的项目文件结构如下，包含了所有配置和安装脚本：
    ![项目文件结构](assets/images/project-structure.png)
    *图：local-ai-ubuntu 项目结构*

3本地运行，不访问任何外部网络服务，无需 API 密钥，零费用。
*   **无大型模型负担:** 不涉及复杂的 AI 计算，资源占用极低，老电脑也能流畅运行。
*   **本地化与隐私:** 所有操作和数据都在你的电脑上完成，无需担心隐私泄露。
*   **极速响应:** 简单的规则匹配和命令执行，几乎没有延迟。
*   **完全可控与透明:** 你清楚地.  **运行一键安装脚本:**
    ```bash
    chmod +x install.sh
    ./install.sh
    ```
...
4.  **首次启动快捷键服务 & 设置开机自启 (重要！):**
    *知道每个短语会触发哪个命令，行为完全可预测。
*   **高度可定制:** 你可以随时添加、   安装完成后，打开一个终端，输入 `sxhkd &` 并回车，以启动快捷键监听。
修改或删除指令规则，打造完全符合你个人习惯的助手。
*   **离线可用:**     *   为了让快捷键在每次开机后都生效，请务必将 `sxhkd` 添加到核心功能无需网络连接。

---

## 功能示例（你可以自定义更多！）

通过编辑核心的 `custom_你 Ubuntu 的“启动应用程序”设置中。

5.  **开始使用:**
    *   按下 `rules.yaml` 文件，你可以让它执行各种操作，例如：

*   **文件和目录操作:**Ctrl + 空格` (默认快捷键)。
    *   在弹出的 Rofi 输入框中输入你的 "在桌面新建文档", "查看下载文件", "删除临时文件"
*   **系统信息查询:** "显示当前时间", "查看内存使用", "看我的IP地址"
*   **应用启动:** "中文指令，例如 `查看磁盘空间`。
    *   按回车，见证奇迹！
    *   打开浏览器", "启动终端", "打开文件管理器"
*   **系统控制:** "关机", "你也可以通过桌面上生成的“自然语言终端”图标启动。

---

## 🛠️ 定制你的重启电脑" (需要 sudo 权限)
*   **常用工具:** "ping百度", "更新软件列表专属命令

`local-ai-ubuntu` 的强大之处在于它的可定制性。想要添加你自己的常用"
*   **(高级)** 甚至可以配置调用 `oathtool` 来生成 2FA 代码！
...
在新安装的 Ubuntu 系统（推荐 22.0命令？非常简单：
...
2.  在 `rules:` 列表下，按照 YAML 格式添加新的条目：
    ...
    4 LTS 或更高版本）上部署 `local-ai-ubuntu` 非常简单：

1.  **确保依赖已安装 (或让脚本尝试安装):**
    你需要 `git`, `golang`, `rofi`,```
    例如，添加一个“备份重要数据”的命令：
    ```yaml
    - pattern: `sxhkd` 等基础工具。不用担心，我们的安装脚本会尝试自动安装它们。

2. "备份我的文档"
      command: "rsync -avh --delete ~/Documents/ /media/mybackupdrive/  **克隆项目仓库:**
    ```bash
    git clone https://github.com/jinv2DocumentsBackup/" # 确保目标路径正确
    ```
3.  保存文件 (在 nano 中按 `Ctrl+O/local-ai-ubuntu.git
    cd local-ai-ubuntu
    ```
    克隆下来的`，回车，然后 `Ctrl+X`)。
...
4我们相信这个小工具能为你的 Ubuntu 日常使用带来巨大的效率提升和乐趣。现在就去 GitHub 克.  **首次启动快捷键服务 & 设置开机自启 (重要！):**
    *   安装完成后，隆项目，动手试试吧！如果你有任何想法、建议或发现了 Bug，欢迎在 GitHub 仓库提交 Issue 或 Pull Request。

让我们一起，用“自然”的方式，指挥我们的电脑！

**👉 再次奉上项目地址打开一个终端，输入 `sxhkd &` 并回车，以启动快捷键监听。
    *   为了：[https://github.com/jinv2/local-ai-ubuntu](https://github.com/让快捷键在每次开机后都生效，请务必将 `sxhkd` 添加到你 Ubuntu 的“jinv2/local-ai-ubuntu)**

---

**出品人：天算AI (Natural Algorithm)**启动应用程序”设置中。

5.  **开始使用:**
    *   按下 `Ctrl + 空格` (默认快捷键)。
    *   在弹出的 Rofi 输入框中输入你的中文指令，例如
